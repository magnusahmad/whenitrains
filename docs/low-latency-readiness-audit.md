...
- `live-settlement-validate --live --order-id ... --reference ...` records explicit CLOB/onchain settlement validation evidence for a filled settlement row, and `low-latency-readiness-report --require-evidence` requires that validation evidence.
- `low-latency-readiness-report --require-evidence` requires the latest stored live CLOB drift scan to be clear, so the production report cannot pass from stale clear evidence or from the absence of open/problem rows alone without evidence that the live scheduler compared local positions to CLOB sellable balances.
...
- `low-latency-verify-evidence-archive --input-dir ...` verifies the manifest identity header, required archive metadata keys and value formats, exactly one `files:` and `checksums:` section, unique manifest metadata/readiness gate keys, exact required entries scoped to the `files:` section with no unexpected files, required non-blank report files with expected report headers, exact unique SHA-256 checksum entries scoped to the `checksums:` section with no unexpected targets, checksum digest format, checksum targets, checksum matches, and exact `all_gates_passed=True` without opening the trading database, and fails with the archived missing-gate list for incomplete evidence bundles.
...
All passed. The combined roadmap verification ran 303 tests under tracemalloc without the previous unclosed-SQLite ResourceWarning cascade.
