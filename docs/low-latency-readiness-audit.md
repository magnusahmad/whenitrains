...
The live log endpoint at `http://192.168.1.23:8765/` was retried again on 2026-05-11 HKT after adding evidence archiving and failed with `curl: (28) Connection timed out after 8003 milliseconds`.
...
- `low-latency-verify-evidence-archive --input-dir ...` verifies the manifest, required report files, required SHA-256 checksum entries, checksum matches, and `all_gates_passed=True` without opening the trading database, and fails with the archived missing-gate list for incomplete evidence bundles.
