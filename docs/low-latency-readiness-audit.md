...
- `low-latency-readiness-report --require-evidence` requires at least one stored `live_clob_drift_scan_clear` risk event, so the production report cannot pass from the absence of open/problem rows alone without evidence that the live scheduler compared local positions to CLOB sellable balances.
