...
- `live-settlement-validate --live --order-id ... --reference ...` records explicit CLOB/onchain settlement validation evidence for a filled settlement row, and `low-latency-readiness-report --require-evidence` requires validation evidence that matches a filled settlement order row.
