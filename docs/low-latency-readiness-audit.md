...
- `low-latency-verify-evidence-archive --input-dir ...` verifies the manifest, unique readiness gate keys, exact required manifest entries with no unexpected files, required non-blank report files, required unique SHA-256 checksum entries, checksum digest format, checksum targets, checksum matches, and exact `all_gates_passed=True` without opening the trading database, and fails with the archived missing-gate list for incomplete evidence bundles.
...
All passed. The combined roadmap verification ran 293 tests under tracemalloc without the previous unclosed-SQLite ResourceWarning cascade.
