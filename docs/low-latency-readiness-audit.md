...
- `low-latency-verify-evidence-archive --input-dir ...` verifies the manifest, required non-blank report files, required SHA-256 checksum entries, checksum targets, checksum matches, and exact `all_gates_passed=True` without opening the trading database, and fails with the archived missing-gate list for incomplete evidence bundles.
