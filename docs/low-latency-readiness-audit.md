...
- `live-readiness-checklist` prints the ordered live evidence commands for network smoke, auth smoke, kill-switch status, minimum-size manual buy/sell, reconciliation, real-account kill-switch verification, capped scheduler smoke, live settlement validation, latency percentiles including submit-to-fill and submit-to-reject, and `low-latency-readiness-report --require-evidence`.
