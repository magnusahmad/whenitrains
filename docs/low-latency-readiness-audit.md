...
All passed. The combined roadmap verification ran 368 tests under tracemalloc after switching generated live readiness commands to `.venv/bin/python`. After the live scheduler log-evidence and scheduler-log download checklist updates, the broader venv discovery command now passes with 455 tests while escalating `ResourceWarning` to an error, confirming the broader dashboard, experiment, hourly, paper, and user WebSocket fixture connections are closed cleanly, the DB-audit usable WebSocket book regression is covered, the HKO timed-response report gate is covered, and verified evidence bundles require `live-scheduler.log`. The archive/report suite now passes 111 tests after requiring `live-scheduler.log` with a successful capped scheduler smoke marker and structured independent-candidate concurrency/no-opportunity evidence. `git diff --check` passed.
