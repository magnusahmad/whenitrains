...
Archive this output with the scheduler logs. It is read-only and does not touch the database. Add `--live-log-url http://<live-host>:8765/` only when the evidence runner can reach the live log server; otherwise keep the generated `--live-log-file <path-to-live-scheduler.log>` archive command and provide the copied log path. The generated archive command records `live_log_url` in `manifest.txt` when `--live-log-url` is supplied.
