...
def _archive_report_fixture_content(name: str) -> str:
    ...
    if name == "readiness_db_audit.txt":
        return (
            ...
            "live_network_smoke_latest=ok\n"
            "live_auth_smoke_records=1\n"
            "live_auth_smoke_latest=ok\n"
            "live_scheduler_smoke_records=1\n"
            "live_scheduler_smoke_latest=ok\n"
...
class LatencyReportTests(unittest.TestCase):
    ...
    def test_low_latency_archive_evidence_copies_scheduler_log_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            manifest = (output_dir / "manifest.txt").read_text()
            self.assertIn(f"live_log_file={source_log}", manifest)
            self.assertIn("- live-scheduler.log", manifest)
            self.assertIn("sha256 live-scheduler.log=", manifest)
    ...
    def test_low_latency_verify_evidence_archive_fails_failed_scheduler_smoke_log(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "live-scheduler.log"
            ...
            report.write_text(
                _archive_report_fixture_content(name).replace(
    ...
    def test_low_latency_verify_evidence_archive_fails_blank_live_log_file_metadata(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            manifest = (output_dir / "manifest.txt").read_text()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive manifest metadata invalid: live_log_file",
                stdout.getvalue(),
            )
    ...
    def test_low_latency_verify_evidence_archive_fails_naive_created_at_metadata(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (output_dir / "manifest.txt").write_text(
                manifest.replace(
                    "created_at_utc=2026-05-11T00:00:00+00:00",
                    "created_at_utc=2026-05-11T00:00:00",
                )
            )
            stdout = StringIO()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive manifest metadata invalid: created_at_utc",
                stdout.getvalue(),
            )

    def test_low_latency_verify_evidence_archive_fails_non_utc_created_at_metadata(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            manifest = (output_dir / "manifest.txt").read_text()
            (output_dir / "manifest.txt").write_text(
                manifest.replace(
                    "created_at_utc=2026-05-11T00:00:00+00:00",
                    "created_at_utc=2026-05-11T08:00:00+08:00",
                )
            )
            stdout = StringIO()
    ...
    def test_low_latency_verify_evidence_archive_fails_duplicate_live_log_file_metadata(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            manifest = (output_dir / "manifest.txt").read_text()
            (output_dir / "manifest.txt").write_text(
                manifest.replace(
                    "hko_limit=200",
                    "hko_limit=200\n"
                    "live_log_file=/tmp/live-scheduler-a.log\n"
                    "live_log_file=/tmp/live-scheduler-b.log",
                )
            )
            stdout = StringIO()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive duplicate manifest key: live_log_file",
                stdout.getvalue(),
            )
    ...
    def test_low_latency_verify_evidence_archive_fails_db_audit_present_with_zero_required_count(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            path = output_dir / "readiness_db_audit.txt"
    ...
    def test_low_latency_verify_evidence_archive_fails_inconsistent_db_audit_smoke_latest(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            path = output_dir / "readiness_db_audit.txt"
            text = path.read_text()
            path.write_text(
                text.replace("live_network_smoke_latest=ok", "live_network_smoke_latest=missing")
            )
            manifest_lines = (output_dir / "manifest.txt").read_text().splitlines()
            updated_manifest = []
            for line in manifest_lines:
    ...
    def test_low_latency_verify_evidence_archive_fails_unknown_missing_gate_name(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            manifest = (output_dir / "manifest.txt").read_text()
    ...
    def test_low_latency_verify_evidence_archive_fails_malformed_manifest_file_entry(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            manifest = (output_dir / "manifest.txt").read_text()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive manifest entry malformed: readiness_report.txt",
                stdout.getvalue(),
            )
    ...
    def test_low_latency_verify_evidence_archive_fails_malformed_checksum_line(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            manifest = (output_dir / "manifest.txt").read_text()
            (output_dir / "manifest.txt").write_text(
                manifest + "sha256 readiness_report.txt\n"
            )
            stdout = StringIO()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive checksum entry malformed: sha256 readiness_report.txt",
                stdout.getvalue(),
            )
    ...
    def test_low_latency_verify_evidence_archive_fails_unexpected_readiness_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                text.replace(
                    "live:\n",
                    "gate unexpected_future_gate=pass count=1\nlive:\n",
                    1,
                )
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate unexpected: unexpected_future_gate",
                stdout.getvalue(),
            )

    def test_low_latency_verify_evidence_archive_fails_malformed_readiness_gate_line(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            report = output_dir / name
            text = report.read_text()
            malformed = "gate live_network_smoke_ok pass count=1\n"
            report.write_text(text.replace("live:\n", malformed + "live:\n", 1))
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            manifest = (output_dir / "manifest.txt").read_text()
            (output_dir / "manifest.txt").write_text(
                "\n".join(
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate malformed: gate live_network_smoke_ok pass count=1",
                stdout.getvalue(),
            )

    def test_low_latency_verify_evidence_archive_fails_duplicate_readiness_gate_field(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                report.read_text().replace(
                    "gate live_network_smoke_ok=pass count=1\n",
                    "gate live_network_smoke_ok=pass count=bad count=1\n",
                    1,
                )
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate malformed: gate live_network_smoke_ok=pass count=bad count=1",
                stdout.getvalue(),
            )

    def test_low_latency_verify_evidence_archive_fails_bare_passing_readiness_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                report.read_text().replace(
                    "gate live_network_smoke_ok=pass count=1\n",
                    "gate live_network_smoke_ok=pass\n",
                    1,
                )
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate malformed: gate live_network_smoke_ok=pass",
                stdout.getvalue(),
            )

    def test_low_latency_verify_evidence_archive_fails_non_numeric_readiness_gate_count(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                report.read_text().replace(
                    "gate live_network_smoke_ok=pass count=1\n",
                    "gate live_network_smoke_ok=pass count=not-a-number\n",
                    1,
                )
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate malformed: gate live_network_smoke_ok=pass count=not-a-number",
                stdout.getvalue(),
            )
    ...
    def test_low_latency_verify_evidence_archive_fails_non_optional_pass_gate_with_zero_count(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                report.read_text().replace(
                    "gate live_network_smoke_ok=pass count=1\n",
                    "gate live_network_smoke_ok=pass count=0\n",
                    1,
                )
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate malformed: gate live_network_smoke_ok=pass count=0",
                stdout.getvalue(),
            )

    def test_low_latency_verify_evidence_archive_fails_malformed_readiness_gate_boolean(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                report.read_text().replace(
                    "gate kill_switch_clear=pass count=1\n",
                    "gate kill_switch_clear=pass block_new_entries=nope exit_on_kill_switch=False\n",
                    1,
                )
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate malformed: gate kill_switch_clear=pass block_new_entries=nope exit_on_kill_switch=False",
                stdout.getvalue(),
            )

    def test_low_latency_verify_evidence_archive_fails_malformed_readiness_gate_latest(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                report.read_text().replace(
                    "gate live_network_smoke_ok=pass count=1\n",
                    "gate live_network_smoke_ok=pass count=1 latest=maybe\n",
                    1,
                )
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate malformed: gate live_network_smoke_ok=pass count=1 latest=maybe",
                stdout.getvalue(),
            )
    ...
    def test_low_latency_verify_evidence_archive_fails_malformed_live_money_state_counter(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                report.read_text().replace(
                    "gate live_money_state_clear=pass count=1\n",
                    "gate live_money_state_clear=pass "
                    "unresolved_orders=none problem_orders=0 submitted=0 error=0 missing_bid_positions=0\n",
                    1,
                )
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate malformed: gate live_money_state_clear=pass unresolved_orders=none",
                stdout.getvalue(),
            )

    def test_low_latency_verify_evidence_archive_fails_malformed_optional_gate_evidence(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                report.read_text().replace(
                    "gate submit_to_reject_observed=pass count=1\n",
                    "gate submit_to_reject_observed=pass evidence=maybe count=1\n",
                    1,
                )
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate malformed: gate submit_to_reject_observed=pass evidence=maybe count=1",
                stdout.getvalue(),
            )

    def test_low_latency_verify_evidence_archive_fails_unknown_readiness_gate_field(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                report.read_text().replace(
                    "gate live_network_smoke_ok=pass count=1\n",
                    "gate live_network_smoke_ok=pass count=1 extra=1\n",
                    1,
                )
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "evidence archive readiness gate malformed: gate live_network_smoke_ok=pass count=1 extra=1",
                stdout.getvalue(),
            )
    ...
    def test_low_latency_verify_evidence_archive_fails_duplicate_readiness_sections(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            report = output_dir / name
            report.write_text(report.read_text() + "evidence gates:\n")
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            manifest = (output_dir / "manifest.txt").read_text()
            (output_dir / "manifest.txt").write_text(
                "\n".join(
    ...
    def test_low_latency_verify_evidence_archive_fails_readiness_report_without_latency_section(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
    ...
    def test_low_latency_verify_evidence_archive_fails_readiness_report_with_missing_footer(self):
        with tempfile.TemporaryDirectory() as tmp:
            output_dir = Path(tmp) / "evidence"
            _write_complete_evidence_archive(output_dir)
            name = "readiness_report.txt"
            ...
            report.write_text(
                report.read_text()
                + "readiness evidence missing: live_network_smoke_ok\n"
            )
            digest = hashlib.sha256(report.read_bytes()).hexdigest()
            manifest = (output_dir / "manifest.txt").read_text()
            (output_dir / "manifest.txt").write_text(
    ...
    def test_low_latency_readiness_report_fails_when_kill_switch_allowed_has_blocking_details(self):
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "test.db"
            db = connect(db_path)
            migrate(db)
            ...
            self.assertEqual(exit_code, 2)
            self.assertIn(
                "gate live_kill_switch_verification=missing count=0 latest=allowed",
                text,
            )
