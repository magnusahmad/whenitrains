...
2026-05-11 production DB read-only evidence audit: `sqlite3 -readonly data/whenitrains.sqlite3` could identify the file but not open it, so a Python `sqlite3.connect('file:data/whenitrains.sqlite3?mode=ro', uri=True)` read-only connection was used for inspection only. The DB has 22,372 HKO `raw_snapshots` rows and 637,810 `orderbook_snapshots` rows, but zero `latency_trace_events`, zero timed HKO raw rows with both `fetch_started_at_utc` and `response_elapsed_ms`, zero `paper_decisions` rows with `orderbook_state_age_seconds`, zero `live_positions`, and no live order/user/risk-event evidence rows. The approved LAN retry for `curl -L --max-time 8 http://192.168.1.23:8765/` still failed with connection refused, so the live evidence exit criteria remain unmet.
