...
2026-05-11 network smoke fail-closed pass: `live-network-smoke --live --require-connected` now exits `2` unless the scheduler-owned WebSocket runtime is alive and every reported client has connected at least once. The runbook and readiness audit now use this stricter no-trade smoke command for live network evidence. Verification covers the previously successful no-trade smoke path and the failed connected-once gate.
