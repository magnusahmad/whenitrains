...
2026-05-11 scheduler-log archive verifier pass: tightened `low-latency-verify-evidence-archive` so final evidence bundles must include `live-scheduler.log` with scheduler start/action lines and capped scheduler smoke outcome. `low-latency-archive-evidence` now includes a pre-downloaded `live-scheduler.log` in the manifest and checksums when the operator places it in the output directory before archiving. Verified red/green with `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -p test_latency_report.py -k scheduler`, then `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -p test_latency_report.py` passing 109 tests.

2026-05-11 post-scheduler-log-archive full verification: `PYTHONWARNINGS=error::ResourceWarning PYTHONTRACEMALLOC=5 PYTHONPATH=src .venv/bin/python -m unittest discover -s tests` passed with 452 tests.
