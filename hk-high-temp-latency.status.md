...
- Low-latency roadmap: current code is a solid scaffold but not yet competitive with passive bots. Next implementation track is `docs/low-latency-readiness-roadmap.md`: add latency instrumentation, DB-change driven decisioning, Polymarket market WebSocket book cache, candidate fan-out with parallel execution across independent markets/strategies, user WebSocket reconciliation, and HKO burst/backoff hardening.
