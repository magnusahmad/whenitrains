...
2026-05-11 live network smoke command pass: added `live-network-smoke --live --seconds N`, which starts the scheduler-owned market/user WebSocket runtime, waits briefly, prints `websocket_all_running`, and stops the runtime without running trading decisions. The live runbook and readiness audit now point to this as the first no-trade network evidence step. Verification covers CLI wiring, runtime start/stop, and that `run_live_tick` is not called.
