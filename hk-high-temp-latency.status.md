...
Low-latency readiness M5 local scaffolding is implemented. Learned AWS actual publish windows now include a 10-second pre/post burst plan with 0.5-second cadence, while broader catchup polling remains. Scheduler source backoff can slow non-critical HKO sources without suppressing `aws_actual` polling. HKO raw snapshots now preserve fetch start time, header receipt time, payload receipt time, and response elapsed milliseconds for source-timing audits. `hko-source-timing-report` summarizes the persisted timing evidence for live dry-run review and now prints `timed_response_rows` separately from total raw HKO rows so missing explicit response timing is visible. Live hot-path buys now fail closed only after a stale/missing candidate WebSocket book cannot be rescued by a bounded targeted CLOB `/book` refresh.
...
Latency reporting can now summarize trace rows directly from the database. `latency-report <start_stage> <end_stage>` prints count plus p50/p95/p99 nearest-rank durations, while `low-latency-readiness-report` combines the core latency pairs, live CLOB drift-scan timing, explicit evidence gates, live order/position counters, kill-switch state, and HKO source-timing evidence. `low-latency-archive-evidence --output-dir ... --live-log-file ... --require-evidence` writes the latency reports, HKO source timing report, readiness output, copies the provided capped scheduler log to `live-scheduler.log`, writes SHA-256 checksums, and records copied-log/optional URL provenance in a manifest for production evidence capture; `low-latency-verify-evidence-archive --input-dir ...` verifies the manifest identity header, required archive metadata keys and value formats, exactly one ordered `files:` and `checksums:` section, unique metadata/gate keys, an exact required manifest file list scoped to the `files:` section, non-blank report files with expected report headers/gate lines, latency p50/p95/p99 fields, positive HKO `timed_response_rows`, HKO public-availability offsets, capped scheduler log start/actions/successful-smoke evidence, structured independent-candidate concurrency/no-opportunity evidence, passing archived readiness gate statuses, exact unique SHA-256 checksum entries scoped to the `checksums:` section, checksum digest format, checksum targets, checksum matches, exact `all_gates_passed=True`, and non-contradictory well-formed `missing_gates` without opening the trading database. Event-keyed live buy/sell execution records `order_submitted`, `clob_ack`, `fill_matched`, `fill_confirmed`, and terminal `order_rejected` stages, allowing live checks for HKO commit-to-decision and submit/fill-or-reject timing.
...
2026-05-14 live dashboard load check: the production-like DB had grown past 32M orderbook snapshots, making `/api/live/stats`, `/api/live/forecast-panels`, and `/api/live/pnl` feel stuck on first load. The dashboard now uses the append-only SQLite sequence for the orderbook snapshot count, scopes production forecast-panel orderbook chart points to the active HKT day, buckets indexed orderbook ranges in Python instead of SQL temp B-trees, replays PnL from minute-bucketed bid snapshots instead of every raw orderbook row, and has additive covering/expression indexes for chart rows, HKO dashboard counts, OCF update-key dedupe, and live order markers. A fresh backup was created at `data/backups/whenitrains-20260514-111809-628395.sqlite3` before applying the schema migration. Local production-like timing improved from roughly 25s stats, 138s YES panels, 90s NO panels, and 48s PnL to roughly 1s stats, 6.4s YES panels, 7.8s NO panels, and 2.8s PnL. Browser verification against `http://127.0.0.1:8770/live` showed the live dashboard rendered with summary tiles, D+0/D+1/D+2 charts, live trade markers, and no console errors.

The scheduler orderbook refresh now fetches independent CLOB token books concurrently, then writes snapshots through the main SQLite connection in deterministic outcome order. The background refresh cadence is now 120 seconds to reduce REST bandwidth and database growth, while live entries still require a fresh WebSocket book or targeted REST refresh in the hot path before submitting. Individual token failures remain non-fatal warnings.
...
2026-05-11 post-venv-docs live log endpoint retry: sandboxed `curl -L --max-time 8 http://192.168.1.23:8765/` timed out after 8 seconds; the approved LAN retry failed immediately with `curl: (7) Failed to connect to 192.168.1.23 port 8765 after 1 ms: Couldn't connect to server`. Live/account evidence capture remains blocked on endpoint availability.

2026-05-11 post-venv-command full roadmap verification: `PYTHONTRACEMALLOC=5 PYTHONPATH=src python3 -m unittest tests.test_runner tests.test_live tests.test_cli tests.test_low_latency tests.test_storage tests.test_markets tests.test_orderbook_cache tests.test_recorded_fixtures tests.test_latency_report tests.test_scheduler tests.test_operational_readiness tests.test_alerting tests.test_live_user_stream tests.test_user_websocket tests.test_execution_scheduler tests.test_candidate_planner tests.test_ladder_metadata` passed with 368 tests after switching generated live readiness commands to `.venv/bin/python`.

2026-05-11 readiness audit latest-verification venv refresh: updated `docs/low-latency-readiness-audit.md` so its latest verification section references the current post-venv-command full roadmap suite.
...
2026-05-11 post-DB-audit-tightening production DB audit: reran `PYTHONPATH=src .venv/bin/python -m whenitrains.cli --db data/whenitrains.sqlite3 low-latency-readiness-db-audit`. It exited `2` with 22,372 HKO raw snapshots and 637,810 orderbook snapshots, but zero latency trace rows, zero latency stage pairs, zero timed HKO rows, zero usable WebSocket orderbook snapshots, zero orderbook-age decisions, zero live orders/user events, and zero live smoke/auth/scheduler/kill-switch/drift/settlement-validation records.

2026-05-11 alternate live log endpoint found: user suggested `192.168.1.49:8765`; `curl -L --max-time 8 http://192.168.1.49:8765/` succeeded and listed live scheduler logs. Downloaded `live-scheduler-20260511-071055.log` to `/private/tmp/whenitrains-live-logs/test.log`; it has 3,332 lines, shows `LIVE TRADING scheduler starting`, startup backup creation, live config/preflight, `live-scheduler started`, and repeated decision loops with `buys=0/0 sells=0/0`. It does not contain live network smoke, auth smoke, manual live buy/sell, settlement validation, or readiness report evidence. Updated the active runbook/checklist/useful commands to use the reachable `.49` endpoint.

2026-05-11 live scheduler WebSocket log detail pass: `live-scheduler --live` startup now logs `live websocket runtime started all_running=<bool> client_count=<n>` after starting the scheduler-owned market/user WebSocket runtime. This improves archived scheduler logs while preserving `live-network-smoke --live --require-connected` as the required connection-evidence gate. Verified red/green with `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -p 'test_cli.py' -k test_live_scheduler_starts_websocket_runtime_and_passes_book_cache_to_ticks`, then `PYTHONWARNINGS=error::ResourceWarning PYTHONTRACEMALLOC=5 PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -p 'test_cli.py'` passing 33 tests.
...
2026-05-11 HKO timed-response report clarity pass: `hko-source-timing-report` now prints `timed_response_rows=<n>` immediately after total HKO source rows, and `low-latency-verify-evidence-archive` rejects archived HKO timing reports with raw rows but zero timed response rows. The production DB currently reports `hko source timing rows=200` and `timed_response_rows=0`, making the missing M5 response-timing evidence explicit instead of hidden behind `response_ms n/a`. Verified red/green with `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -p test_cli.py -k test_hko_source_timing_report_summarizes_aws_fetch_attempts` and `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -p test_latency_report.py -k low_latency_verify_evidence_archive`.

2026-05-11 post-HKO-timed-response full verification: `PYTHONWARNINGS=error::ResourceWarning PYTHONTRACEMALLOC=5 PYTHONPATH=src .venv/bin/python -m unittest discover -s tests` passed with 450 tests.

2026-05-11 scheduler-log archive verifier pass: tightened `low-latency-verify-evidence-archive` so final evidence bundles must include `live-scheduler.log` with scheduler start/action lines and a successful capped scheduler smoke outcome. `low-latency-archive-evidence` now includes a pre-downloaded `live-scheduler.log` in the manifest and checksums when the operator places it in the output directory before archiving, and a failed scheduler-smoke log is rejected. Verified red/green with `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -p test_latency_report.py -k scheduler`, then `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -p test_latency_report.py` passing 110 tests.

2026-05-11 post-scheduler-log-archive full verification: `PYTHONWARNINGS=error::ResourceWarning PYTHONTRACEMALLOC=5 PYTHONPATH=src .venv/bin/python -m unittest discover -s tests` passed with 453 tests.

2026-05-11 scheduler concurrency evidence log pass: capped `live-scheduler --live --ticks N` now prints `live scheduler concurrency evidence ...` before the smoke marker. The scheduler loop returns aggregate candidate-action counts across the capped run; the live scheduler logs `independent_candidate_opportunity=not_observed` only when fewer than two candidate actions occurred, otherwise it refuses to claim concurrency proof. The archive verifier now requires either that no-opportunity line or an explicit `independent_candidate_concurrency=observed` line in `live-scheduler.log`. Verified red/green with focused scheduler/CLI/archive tests, then `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -p test_scheduler.py` passing 36 tests, `test_cli.py` passing 33 tests, and `test_latency_report.py` passing 111 tests.
...
2026-05-11 non-LAN archive smoke: ran `low-latency-archive-evidence --output-dir /private/tmp/whenitrains-nonlan-evidence --live-log-file /private/tmp/whenitrains-live-scheduler-latest.log --require-evidence` against `data/whenitrains.sqlite3`. It copied the scheduler log into the archive, wrote `live_log_file=/private/tmp/whenitrains-live-scheduler-latest.log`, added `live-scheduler.log` to the manifest and checksums, and exited `2` with the expected missing readiness gates. `low-latency-verify-evidence-archive --input-dir /private/tmp/whenitrains-nonlan-evidence` also exited `2`, rejecting the zero-sample latency reports, failed DB audit, missing HKO timed-response/live/account gates, and the old scheduler log that lacks capped-smoke/concurrency markers.

2026-05-11 non-LAN completion audit refresh: reran `low-latency-readiness-db-audit` and `low-latency-readiness-report --require-evidence` read-only against `data/whenitrains.sqlite3`. Both still exit `2` because the local DB has zero required latency-stage pairs, zero timed HKO raw snapshots, zero usable WebSocket orderbook snapshots, zero paper decisions with orderbook age, zero manual/reconciled/settlement live orders, zero user-channel events/trades, zero live drift/kill-switch/settlement-validation evidence, and the latest recorded network/auth/scheduler smoke attempts are failed. This confirms the remaining roadmap exit criteria are live/account evidence capture, not local implementation work; because this workstation is no longer on the live LAN, future scheduler-log evidence must be copied into the archive via `--live-log-file` or collected through a reachable `--live-log-url`.
...
2026-05-11 M1 DB-change decisioning re-audit: rechecked HKO actual, OCF forecast-sample, and Polymarket market-resolution enqueue-after-commit paths; blocking `FastDecisionWorker` dispatch through a separate SQLite connection; queue wakeup; scheduler drain-before-watchdog/default dispatch; retryability when orderbooks are missing; and processed-event idempotency. Verified `test_low_latency.py` passing 11 tests, scheduler fast-event filters passing 3 tests, runner processed-once filters passing 4 tests, runner missing-orderbook retry filters passing 3 tests, and the duplicate-HKO-update runner filter passing 1 test. The remaining M1 exit criterion is live-machine proof that row commit to decision start is consistently below 1 second.

2026-05-11 M4 user reconciliation re-audit: rechecked authenticated user WebSocket subscription/application, independent lifecycle-event storage, idempotent matched-trade position deltas, restart convergence from a later user trade, REST pending-order reconcile, local/CLOB sellable-balance drift detection/repair/freeze, local resolved-market settlement, settlement validation command, and readiness gates for user events, applied trades, reconcile evidence, settlement rows, settlement validation, and clear drift scans. Verified `test_live_user_stream.py` passing 4 tests, `test_user_websocket.py` passing 3 tests, live reconcile filters passing 7 tests, live drift filters passing 3 tests, settlement-validation CLI filters passing 2 tests, live-scheduler reconcile-watchdog filters passing 3 tests, full `test_runner.py` passing 86 tests, and full `test_latency_report.py` passing 119 tests. The remaining M4 exit criteria are real user WebSocket smoke, recent-trades validation, real-account drift proof, and CLOB/onchain settlement validation evidence.

2026-05-11 M5 polling hardening re-audit: rechecked learned AWS actual burst windows and 0.5-second cadence, non-critical source backoff isolation from AWS actual polling, source-freshness breach alerting, HKO fetch/header/payload/elapsed timing capture, raw snapshot timing persistence, `hko-source-timing-report`, readiness gates for timed HKO rows and public-availability clustered fetches, and live hot-path stale WebSocket book rejection. Verified scheduler burst/backoff/freshness filters passing 1 test each, stale WebSocket book-cache runner filter passing 1 test, `test_hko.py` passing 10 tests, raw-snapshot storage filters passing 2 tests, HKO source-timing CLI filter passing 1 test, and HKO readiness/archive filters in `test_latency_report.py` passing 9 tests. The remaining M5 exit criterion is captured live dry-run timing evidence from the production DB showing clustered fetches and sub-second commit-to-decision.
...
- Run `low-latency-archive-evidence --output-dir data/low-latency-evidence/<run-id> --live-log-file <path-to-live-scheduler.log> --require-evidence` on the production DB, verify it with `low-latency-verify-evidence-archive --input-dir data/low-latency-evidence/<run-id>`, and archive the generated artifacts with copied scheduler-log provenance.
...
2026-05-12 targeted orderbook refresh pass: live entry execution no longer treats a stale or missing candidate token WebSocket book as an automatic miss. If the scheduler-owned cache cannot provide a book under the 250 ms freshness cap, the hot path performs one targeted CLOB `/book` refresh for that token with a 500 ms timeout, persists the refreshed snapshot with `rest_targeted` metadata, seeds the cache, and then re-applies the existing entry cap and slippage checks before submitting. If the targeted refresh fails, the decision is still recorded as missed with `targeted orderbook refresh failed: ...`. This is intended to rescue quiet-token opportunities like the 2026-05-13 D+1 low `26°C YES` move while preserving fail-closed behavior for unavailable fresh depth. Verified with focused live runner tests plus `tests.test_runner`, `tests.test_orderbook_cache`, and the live-scheduler WebSocket CLI smoke wiring test.

2026-05-14 background orderbook cadence pass: changed the scheduler's default full-sweep CLOB orderbook refresh interval from 15 seconds to 120 seconds to reduce bandwidth and append-only DB growth. The live buy hot path remains unchanged: it uses a WebSocket book under the 250 ms freshness cap or performs a targeted REST refresh for the candidate token before submitting, and misses the trade if that fresh check fails. Red/green verification added `test_orderbooks_default_to_two_minute_cadence`; verified with `test_scheduler.py` and focused live runner tests for targeted/WebSocket refresh behavior.

2026-05-14 actual-cross boundary guard pass: live audit of the 19:20 HKT lowest-temperature `26°C YES` buy showed the actual-low-cross path allowed exact-bucket YES entries when the effective low forecast sat within a small buffer of the bucket's lower invalidation boundary. Added symmetric exact-bucket forecast-boundary guards for actual low-cross and high-market actual-cross YES entries, using the existing 0.20C near-boundary margin; examples now cover low `26°C YES` with forecast low `26.1` and high `26°C YES` with forecast high `26.8`. Invalidated-side NO buys remain allowed. Red/green verification added `test_actual_low_cross_forecast_boundary_guard_skips_exact_yes_near_lower_boundary` and `test_actual_cross_forecast_boundary_guard_skips_exact_yes_near_upper_boundary`; verified with `PYTHONPATH=src .venv/bin/python -m unittest discover -s tests -p 'test_runner.py'`.
