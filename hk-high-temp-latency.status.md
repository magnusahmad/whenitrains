...
2026-05-11 blocking fast-worker pass: added `FastDecisionWorker`, which blocks on `LowLatencyEventQueue`, owns a separate SQLite connection, dispatches the same narrow fast-event handlers, and records decision start/completion stages without waiting for the scheduler loop. Verification covers a worker processing a queued forecast event and writing latency stages from its own connection.
